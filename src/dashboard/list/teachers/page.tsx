...
export default function TeachersList() {
	...
	React.useEffect(() => {
		...
		const fetchData = async () => {
			setLoading(true);
			await Promise.all([fetchTeachers(), fetchStatusOptions()]);
			setLoading(false);
		};
	...
	const filteredItems = React.useMemo(() => {
		...
		if (
			...
			filteredTeachers = filteredTeachers.filter((teacher) =>
				Array.from(statusFilter).includes(teacher.status)
			);
		}

		return filteredTeachers;
	}, [teachers, filterValue, statusFilter, hasSearchFilter, statusOptions]);
	...
	const sortedItems = React.useMemo(() => {
		return [...items].sort((a: Teacher, b: Teacher) => {
			const first = a[sortDescriptor.column as keyof Teacher] as number;
			const second = b[sortDescriptor.column as keyof Teacher] as number;
	...
	const renderCell = React.useCallback(
		(teacher: Teacher, columnKey: React.Key) => {
	...
	}, [selectedKeys, items.length, page, pages, hasSearchFilter, loading]);
